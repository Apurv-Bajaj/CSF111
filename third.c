...
n->d=dp; n->next=NULL;
if(d_list==NULL) { d_list=n; n->next=NULL; return d_list;};

if(dp.deg==1){ n->next=d_list; d_list=n; return d_list;}
...
int c=0;LINK temp=list;
while(temp!=NULL)
{
if(strcmp(dname,temp->d.dept_name)==0) c+=temp->d.no_of_faculty;
temp=temp->next;
}
